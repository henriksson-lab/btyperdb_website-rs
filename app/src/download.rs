...
impl Model {
    ...
    pub fn download_metadata(&self, list_strains: &Vec<String>) {
        ...
        let blob = Blob::new_with_buffer_source_sequence_and_options(&blob_parts, &blob_properties)
            .unwrap();
        ...
        let a: HtmlElement = document
            .create_element("a")
            .expect("could not create a")
            .dyn_into()
            .unwrap();
        ...
        a.set_attribute("download", "metadata.txt")
            .expect("Could not set attribute");
    ...
    pub fn download_fasta(&self, data: &Vec<u8>) {
        ...
        // a.setAttribute('download', 'download.csv');
        //    a.set_attribute("download", "fastq.zip").expect("Could not set attribute");
        a.set_attribute("download", "btyper_fasta.zip")
            .expect("Could not set attribute");
