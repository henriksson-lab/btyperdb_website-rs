...
impl TreeLayout {
    ...
    pub fn new(newick_str: &str) -> TreeLayout {
        ...
        let tree: SimpleRootedTree<String, f32, f32> =
            PhyloTree::from_newick(newick_str.as_bytes()).expect("Could not parse tree");
    ...
    ////////////////////////////////////////////////////////////
    /// x
    fn calc_x<T, W, Z>(tree: &SimpleRootedTree<T, W, Z>) -> Vec<W>
    where
        T: NodeTaxa,
        W: EdgeWeight,
        Z: NodeWeight,
    ...
    ////////////////////////////////////////////////////////////
    /// x
    fn calc_x_recursive<T, W, Z>(
        tree: &SimpleRootedTree<T, W, Z>,
        list_x: &mut Vec<W>,
        cur_node: usize,
        cur_x: W,
    ) where
        ...
        Z: NodeWeight,
    {
        for child_id in tree.get_node_children_ids(cur_node) {
            let w = tree
                .get_edge_weight(cur_node, child_id)
                .expect("no edge weight");
            let next_w = cur_x + w;
